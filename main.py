...
def main(page: ft.Page):
    ...
    splash = ft.Container(content=ft.Column([ft.Icon("sports_soccer", size=80, color=COR_ACCENT), ft.Text("AtletiQ 2.1", size=40, weight="bold"), pb, txt_load], alignment="center", horizontal_alignment="center"), expand=True)
    ...
    if not dfs_finais:
        txt_load.value = "Erro: Sem dados (Internet ou Cache falhou)."; page.update()
        return
    ...
    def abrir_detalhes(row):
        ...
        modal = ft.BottomSheet(
            ft.Container(
                content=ft.Column([
                    ft.Row([ft.Text("Match Center", size=18, weight="bold"), ft.IconButton(ft.Icons.CLOSE, on_click=fechar)], alignment="spaceBetween"),
    ...
    if not df_fut.empty:
        ...
        for r in sorted(df_fut['Rodada_Num'].dropna().unique()):
            ...
            for _, row in jogos_r.iterrows():
                card = criar_card(ft.Row([ft.Column([ft.Text(row['Date'].strftime("%d/%m - %H:%M"), size=10, color=COR_TEXT_SEC), ft.Row([ft.Text(row['HomeTeam'], size=13, weight="bold", expand=True, text_align="right"), ft.Text("vs", size=10, color=COR_TEXT_SEC), ft.Text(row['AwayTeam'], size=13, weight="bold", expand=True, text_align="left")], spacing=10)], expand=True), ft.Icon(ft.Icons.CHEVRON_RIGHT, color=COR_TEXT_SEC, size=16)], alignment="center"), padding=12, on_click=lambda _, r=row: abrir_detalhes(r))
    ...
    # Aba 2: Artilharia do Campeonato
    dd_time_art = ft.Dropdown(label="Filtrar por Clube", options=[ft.dropdown.Option("Todos")] + [ft.dropdown.Option(t) for t in times_list], value="Todos", expand=True)
    lista_artilharia = ft.Column()

    def atualizar_artilharia(e):
        if df_artilharia_completa is None: return
    ...
    btn_filtro_art = ft.IconButton("search", on_click=atualizar_artilharia, tooltip="Aplicar Filtro")
    btn_limpar_art = ft.IconButton("refresh", on_click=limpar_filtro_artilharia, tooltip="Limpar Filtro", icon_color=COR_TEXT_SEC)
    ...
    tab_artilharia = ft.Container(
        content=ft.Column([
            criar_card(ft.Row([dd_time_art, btn_filtro_art, btn_limpar_art], spacing=10)),
            lista_artilharia
    ...
    area_sim = ft.Column()
    def rodar(e):
        btn_s.content = ft.ProgressRing(width=20, color="black"); page.update()
        ...
        area_sim.controls = [criar_tabela_estilizada(res)]; btn_s.content = ft.Text("SIMULAR CAMPEONATO"); page.update()
    btn_s = ft.ElevatedButton("SIMULAR CAMPEONATO", bgcolor=COR_ACCENT, color="black", on_click=rodar, width=float('inf'))
    tab_sim = ft.Container(content=ft.Column([btn_s, area_sim], scroll=ft.ScrollMode.AUTO), padding=20)

    header = ft.Container(
        content=ft.Row([
            ft.Row([ft.Icon("sports_soccer", color=COR_ACCENT), ft.Text("AtletiQ 2.1", size=22, weight="bold", color="white")]),
            # ft.Text("v2.1", size=10, color=COR_TEXT_SEC)
        ], alignment="spaceBetween"),
        padding=15, border=ft.border.only(bottom=ft.border.BorderSide(1, COR_BORDER))
    )
